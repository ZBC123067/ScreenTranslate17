...
- (UIImage *)captureCurrentAppWindow {
    ...
    void (^capture)(void) = ^{
        ...
        if (![window drawViewHierarchyInRect:window.bounds afterScreenUpdates:NO]) [window.layer renderInContext:context];
...
- (void)recognizeImage:(UIImage *)image regionInImage:(CGRect)region fast:(BOOL)fast completion:(STOCRCompletion)completion {
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        ...
        request.minimumTextHeight = fast ? 0.020 : 0.012;
        if (@available(iOS 16.0, *)) request.automaticallyDetectsLanguage = YES;
        NSArray<NSString *> *desiredLanguages = @[ @"en-US", @"zh-Hans", @"ms-MY", @"id-ID", @"ja-JP", @"ko-KR", @"vi-VN", @"th-TH" ];
        NSArray<NSString *> *supportedLanguages = [VNRecognizeTextRequest supportedRecognitionLanguagesForTextRecognitionLevel:request.recognitionLevel revision:request.revision error:nil];
        if (supportedLanguages.count) {
            NSMutableArray<NSString *> *availableLanguages = [NSMutableArray array];
            for (NSString *language in desiredLanguages) if ([supportedLanguages containsObject:language]) [availableLanguages addObject:language];
