...
require("Version: 0.4.5" in control and "ScreenTranslate17 0.4.5" in readme, "version strings are inconsistent")
require(info.get("CFBundleShortVersionString") == "0.4.5", "preference bundle version is inconsistent")
...
entry_data = plistlib.loads((ROOT / "Preferences/entry.plist").read_bytes())
entry_spec = entry_data.get("entry", {}) if isinstance(entry_data, dict) else {}
require(entry_spec.get("bundle") == "ScreenTranslate17Prefs" and entry_spec.get("detail") == "STRootListController", "PreferenceLoader entry is invalid")
...
require(entry_spec.get("icon") == "/Library/PreferenceLoader/Preferences/ScreenTranslate17.png", "PreferenceLoader entry icon path is incorrect")
...
require("!key.length" in controller and "stored[key]" in controller, "settings controller does not safely handle keyless specifiers")
require("Preferences/Resources/icon.png" in makefile and "ScreenTranslate17.png" in makefile, "PreferenceLoader icon staging is missing")
