...
- (void)translateText:(NSString *)text source:(NSString *)source target:(NSString *)target completion:(STTranslationCompletion)completion {
    ...
    BOOL shouldCache = prepared.tokens.count == 0;
    if (shouldCache) {
...
- (void)translateWithOpenAI:(NSString *)text source:(NSString *)source target:(NSString *)target completion:(STProviderCompletion)completion {
    ...
    NSString *system = [NSString stringWithFormat:@"You are a precise translation engine. Translate from %@ to %@. Preserve names, vessel/voyage, ports, dates, currencies, formatting, and placeholders such as [[0]]. Return only the translation. %@", source, target, [STPrivacy shippingGlossaryInstructionForTargetLanguage:target]];
    NSDictionary *body = @{
...
- (void)translateWithDeepL:(NSString *)text source:(NSString *)source target:(NSString *)target completion:(STProviderCompletion)completion {
    ...
    if (!url) { completion(nil, urlError); return; }
    NSMutableDictionary *body = [@{ @"text": @[ text ], @"target_lang": [self deepLLanguage:target] } mutableCopy];
    if (source.length && ![source.lowercaseString isEqualToString:@"auto"]) body[@"source_lang"] = [self deepLLanguage:source];
