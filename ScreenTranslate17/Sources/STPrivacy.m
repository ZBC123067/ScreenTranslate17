...
+ (BOOL)isCurrentBundleBlocked {
    ...
    for (NSString *blocked in STPreferences.shared.blockedBundleIDs) {
...
+ (BOOL)isSensitiveInputView:(UIView *)view {
    ...
    NSString *label = [[NSString stringWithFormat:@"%@ %@", view.accessibilityLabel ?: @"", view.accessibilityHint ?: @""] lowercaseString];
...
+ (BOOL)view:(UIView *)view containsSensitiveContentWithTerms:(NSArray<NSString *> *)terms {
    ...
    else if ([view isKindOfClass:UITextField.class]) text = [text stringByAppendingFormat:@" %@", ((UITextField *)view).placeholder ?: @""];
...
+ (STPreparedText *)prepareTextForNetwork:(NSString *)text {
    ...
    for (NSTextCheckingResult *match in matches.reverseObjectEnumerator) {
        ...
        NSString *token = [NSString stringWithFormat:@"[[%lu]]", (unsigned long)index++];
