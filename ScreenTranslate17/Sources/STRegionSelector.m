...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    if (self.selected.size.width >= 16.0 && self.selected.size.height >= 16.0) {
        [self finishWithRegion:self.selected cancelled:NO];
    } else {
        self.selected = CGRectZero;
        self.hintLabel.text = @"区域太小，请重新拖动框选";
        [self updateMask];
    }
}
- (void)touchesCancelled:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    self.selected = CGRectZero;
    self.hintLabel.text = @"手势已中断，请重新框选";
    [self updateMask];
}
@end

@interface STRegionSelector ()
@property (nonatomic, strong) STRegionSelectionView *activeSelection;
@end
...
- (void)presentInView:(UIView *)view completion:(STRegionSelectionCompletion)completion {
    ...
    if (self.activeSelection.superview) {
        if (completion) completion(CGRectZero, YES);
        return;
    }
    ...
    selection.completion = ^(CGRect region, BOOL cancelled) {
        weakSelf.activeSelection = nil;
        if (completion) completion(region, cancelled);
    };
