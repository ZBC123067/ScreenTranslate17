...
- 修复 `Root.plist` 顶层结构：Preferences 只会从根字典的 `items` 数组读取设置项。此前虽然 XML 合法、文件也被打包，但顶层直接使用数组，因而会显示空白页面。
- 构建检查现在同时验证最终安装包内的 `Root.plist` 含有标题和 26 个 `items`，防止此类“编译成功、设置页为空”问题再次漏检。

## v0.4.5 设置页内容与图标修复

- 读取偏好值时跳过没有 `key` 的分组和说明项，避免设置框架以空键访问字典而在打开页面时退出。
- 设置列表图标改为从 PreferenceLoader 目录加载，并在打包时明确复制图标到该目录。
